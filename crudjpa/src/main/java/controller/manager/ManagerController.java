...
public class ManagerController extends HttpServlet {
    ...
    private UserService userService = new UserServiceImpl(); // cần để load list user
    private final String UPLOAD_DIR = "F:/NĂM 3/LẬP TRÌNH WEB/JPA/Uploads";
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
        ...
        if (action == null || action.equals("/home")) {
            ...
            req.getRequestDispatcher("/views/manager/manager-add.jsp").forward(req, resp);
        } else if (action.equals("/update")) {
            int id = Integer.parseInt(req.getParameter("id"));
            Category cate = cateService.findById(id);
            req.setAttribute("category", cate);
            req.getRequestDispatcher("/views/manager/manager-update.jsp").forward(req, resp);
        } else {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
        ...
        if(action.equals("/add") || action.equals("/update")) {
            ...
            if(action.equals("/add")) {
                ...
                int id = Integer.parseInt(req.getParameter("id"));
                Category cate = cateService.findById(id);
                // Optional: chỉ cho phép sửa category do chính họ tạo
            ...
            int id = Integer.parseInt(req.getParameter("id"));
            Category cate = cateService.findById(id);
            // Manager chỉ xóa category do chính họ tạo
            if(cate.getUser().getId() != currentUser.getId()) {
                resp.sendError(HttpServletResponse.SC_FORBIDDEN);
                return;
            }
